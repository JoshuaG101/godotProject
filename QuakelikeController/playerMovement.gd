...
#Dashing
@export var dashSpeed : float = 80.0
@export var dashDuration : float = 0.2
@export var dashAccel : float = 30.0
...
#Wall Jump
@export var wallJumpUpForce : float = 10.0
@export var wallJumpPushForce : float = 12.0
...
@onready var realCamera: Camera3D = $Head/CameraHolder/CameraShaker/CameraPivot/SpringArm3D/Camera
...
func _physics_process(delta : float) -> void:
	match currentState:
		...
		MOVESTATES.DASHING:
			dash(delta)
	if Input.is_action_just_pressed("dash") and canDash and not isCrouching:
		startDash()
	
	if jumpCooldownTimer > 0.0:
...
func startDash() -> void:
	...
	dashDir = Vector3(h_input, 0, f_input).rotated(Vector3.UP, h_rot).normalized()
	# If pressing no inputs, dash forward based on camera/body facing
	if dashDir == Vector3.ZERO:
		dashDir = -global_transform.basis.z.normalized()
	changeState(MOVESTATES.DASHING)
	
func dash(delta : float) -> void:
	dashTimer += delta
	var target_vel = dashDir * dashSpeed
	velocity = lerp(velocity, target_vel, dashAccel * delta)
...
func changeState(newState : MOVESTATES) -> void:
	...
	if previousState == MOVESTATES.DASHING:
		emit_signal("endDash")
	if currentState == MOVESTATES.DASHING:
