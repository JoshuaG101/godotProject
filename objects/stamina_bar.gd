...
func _setup_health_bar(max_val: float):
	...
	value = max_val
	max_value = max_val
	$ProgressBar2.value = max_val
	$ProgressBar2.max_value = max_val
...
func change_value(new_value: float):
	...
	change_value_tween.tween_property($ProgressBar2, "value", new_value, 0.35).set_trans(Tween.TRANS_SINE)	
